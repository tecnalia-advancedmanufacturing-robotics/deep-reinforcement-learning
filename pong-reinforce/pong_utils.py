...
def animate_frames(frames):
    ...
    fanim = animation.FuncAnimation(plt.gcf(),
                                    lambda x: patch.set_data(frames[x]), frames=len(frames), interval=60)
...
def play(env, policy, time=2000, preprocess=None, nrand=5):
    ...
    for _ in range(nrand):
        frame1, reward1, d1, d2, _ = env.step(np.random.choice([RIGHT, LEFT]))
        frame2, reward2, d1, d2, _ = env.step(0)
    ...
    for _ in range(time):
        ...
        frame1, _, d1, d2, _ = env.step(action)
        frame2, _, d1, d2, _ = env.step(0)
